...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isMockMode) {
      ...
      // Mock 토큰 설정
      response.cookies.set('access_token', 'mock_google_access_token', {
        httpOnly: true,
        secure: process.env.NODE_ENV === 'production',
        sameSite: 'lax',
        maxAge: 60 * 60 * 24 * 7, // 7일
      })
      response.cookies.set('refresh_token', 'mock_google_refresh_token', {
        httpOnly: true,
        secure: process.env.NODE_ENV === 'production',
        sameSite: 'lax',
    ...
    if (!tokenResponse.ok) {
      return NextResponse.json(
        { success: false, message: 'Google 인증에 실패했습니다.' },
