...
test.describe('Authentication Flow', () => {
  test('should display login page correctly', async ({ page }) => {
    ...
    await expect(page.getByText('로그인하여 리딩타운의 모든 서비스를 이용하세요')).toBeVisible()
  ...
  test('should have correct layout on mobile viewport', async ({ page }) => {
    ...
    // 컨테이너 최대 너비 확인 (430px)
    const container = page.locator('div').filter({ hasText: '리딩타운' }).first()
  ...
  test('should navigate to main page when not on login page', async ({ page }) => {
    await page.goto('/')
